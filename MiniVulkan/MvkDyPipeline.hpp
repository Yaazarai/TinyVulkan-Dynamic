...
#define MINIVK_MINIVKDYNAMICPIPELINE
	...
	namespace MINIVULKAN_NS {
		...
		public:
			/// GRAPHICS_PIPELINE ///
			MvkVertexDescription vertexDescription;

			VkDescriptorSetLayout descriptorLayout;
			std::vector<VkDescriptorSetLayoutBinding> descriptorBindings;

			std::vector<VkPushConstantRange> pushConstantRanges;
			MvkShaderStages& shaderStages;
			...
			void Disposable() {
				...
				vkDestroyDescriptorSetLayout(mvkInstance.logicalDevice, descriptorLayout, nullptr);
				vkDestroyPipeline(mvkInstance.logicalDevice, graphicsPipeline, nullptr);
				vkDestroyPipelineLayout(mvkInstance.logicalDevice, pipelineLayout, nullptr);
			}

			MvkDyPipeline(MvkInstance& mvkInstance, VkFormat imageFormat, MvkShaderStages& shaderStages, MvkVertexDescription vertexDescription, const std::vector<VkDescriptorSetLayoutBinding>& descriptorBindings, const std::vector<VkPushConstantRange>& pushConstantRanges, VkColorComponentFlags colorComponentFlags = VKCOMP_RGBA, VkPrimitiveTopology vertexTopology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST)
			: mvkInstance(mvkInstance), shaderStages(shaderStages), vertexDescription(vertexDescription), imageFormat(imageFormat), colorComponentFlags(colorComponentFlags), vertexTopology(vertexTopology), descriptorBindings(descriptorBindings), pushConstantRanges(pushConstantRanges) {
				onDispose += std::callback<>(this, &MvkDyPipeline::Disposable);

				MvkQueueFamily indices = MvkQueueFamily::FindQueueFamilies(mvkInstance.physicalDevice, mvkInstance.presentationSurface);
				vkGetDeviceQueue(mvkInstance.logicalDevice, indices.graphicsFamily.value(), 0, &graphicsQueue);
				vkGetDeviceQueue(mvkInstance.logicalDevice, indices.presentFamily.value(), 0, &presentQueue);

				CreateGraphicsPipeline();
			}

			MvkDyPipeline operator=(const MvkDyPipeline& pipeline) = delete;
			...
			void CreateGraphicsPipeline() {
				...
				if (descriptorBindings.size() > 0) {
					...
					if (vkCreateDescriptorSetLayout(mvkInstance.logicalDevice, &descriptorCreateInfo, nullptr, &descriptorLayout) != VK_SUCCESS)
				...
				if (vkCreatePipelineLayout(mvkInstance.logicalDevice, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
				...
				if (vkCreateGraphicsPipelines(mvkInstance.logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
			...
			VkFormat QueryDepthFormat(VkImageTiling tiling = VK_IMAGE_TILING_OPTIMAL, VkFormatFeatureFlags features = VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
				...
				for (VkFormat format : candidates) {
					...
					vkGetPhysicalDeviceFormatProperties(mvkInstance.physicalDevice, format, &props);
