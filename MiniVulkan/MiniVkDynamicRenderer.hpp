...
#define MINIVULKAN_MINIVKDYNAMICRENDERER
	...
	namespace MINIVULKAN_NAMESPACE {
		...
		public:
			...
			MiniVkInvokable<VkCommandBuffer> onRenderEvents;
			...
			void Disposable() {
				...
				for (size_t i = 0; i < swapChain_inFlightFences.size(); i++) {
					vkDestroySemaphore(renderDevice.logicalDevice, swapChain_imageAvailableSemaphores[i], nullptr);
					vkDestroySemaphore(renderDevice.logicalDevice, swapChain_renderFinishedSemaphores[i], nullptr);
					vkDestroyFence(renderDevice.logicalDevice, swapChain_inFlightFences[i], nullptr);
				}
			...
			depthImage(renderDevice, memAlloc, swapChain.swapChainExtent.width, swapChain.swapChainExtent.height, true, QueryDepthFormat(), VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, VK_SAMPLER_ADDRESS_MODE_REPEAT, VK_IMAGE_ASPECT_DEPTH_BIT) {
				onDispose += MiniVkCallback<>(this, &MiniVkDynamicRenderer::Disposable);
			...
			void CreateSwapChainSyncObjects() {
				...
				for (size_t i = 0; i < swapChain.swapChainImages.size(); i++) {
					if (vkCreateSemaphore(renderDevice.logicalDevice, &semaphoreInfo, nullptr, &swapChain_imageAvailableSemaphores[i]) != VK_SUCCESS ||
						vkCreateSemaphore(renderDevice.logicalDevice, &semaphoreInfo, nullptr, &swapChain_renderFinishedSemaphores[i]) != VK_SUCCESS ||
						vkCreateFence(renderDevice.logicalDevice, &fenceInfo, nullptr, &swapChain_inFlightFences[i]) != VK_SUCCESS)
			...
			/// <summary>Begins recording commands into a command buffers.</summary>
			void BeginRecordCommandBufferSwapChain(VkCommandBuffer commandBuffer, const VkClearValue clearColor, const VkClearValue depthStencil) {
				VkCommandBufferBeginInfo beginInfo{};
				beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
				beginInfo.flags = VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT;
				beginInfo.pInheritanceInfo = nullptr; // Optional
				...
				VkRenderingAttachmentInfoKHR colorAttachmentInfo{};
				colorAttachmentInfo.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR;
				colorAttachmentInfo.imageView = swapChain.CurrentImageView();
				colorAttachmentInfo.imageLayout = VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL;
				colorAttachmentInfo.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
				colorAttachmentInfo.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
				colorAttachmentInfo.clearValue = clearColor;
				...
				VkRect2D renderAreaKHR{};
				renderAreaKHR.extent = swapChain.CurrentExtent2D();
				renderAreaKHR.offset = { 0,0 };
				dynamicRenderInfo.renderArea = renderAreaKHR;
				dynamicRenderInfo.layerCount = 1;
				dynamicRenderInfo.colorAttachmentCount = 1;
				...
				dynamicViewportKHR.x = 0;
				dynamicViewportKHR.y = 0;
				dynamicViewportKHR.width = static_cast<float>(renderAreaKHR.extent.width);
				dynamicViewportKHR.height = static_cast<float>(renderAreaKHR.extent.height);
				dynamicViewportKHR.minDepth = 0.0f;
				dynamicViewportKHR.maxDepth = 1.0f;
			...
			/// <summary>Ends recording to the command buffer.</summary>
			void EndRecordCommandBufferSwapChain(VkCommandBuffer commandBuffer, const VkClearValue clearColor, const VkClearValue depthStencil) {
				if (vkCmdEndRenderingEKHR(renderDevice.instance.instance, commandBuffer) != VK_SUCCESS)
					throw std::runtime_error("MiniVulkan: Failed to record [end] to rendering!");
			...
			void RenderFrame() {
				...
				vkWaitForFences(renderDevice.logicalDevice, 1, &swapChain_inFlightFences[swapChain.currentFrame], VK_TRUE, UINT64_MAX);

				uint32_t imageIndex;
				VkResult result = vkAcquireNextImageKHR(renderDevice.logicalDevice, swapChain.swapChain, UINT64_MAX, swapChain_imageAvailableSemaphores[swapChain.currentFrame], VK_NULL_HANDLE, &imageIndex);
				...
				vkResetFences(renderDevice.logicalDevice, 1, &swapChain_inFlightFences[swapChain.currentFrame]);
