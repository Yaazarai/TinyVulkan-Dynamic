...
#define MINIVK_MINIVKDYNAMICRENDERER
	...
	namespace MINIVULKAN_NS {
		...
		private:
			...
			MvkMemAlloc& memAlloc;

		public:
			MvkSwapChain& swapChain;
			MvkDyPipeline& graphicsPipeline;
			...
			/// INVOKABLE RENDER EVENTS: (executed in MvkDyRenderer::RenderFrame()
			std::invokable<VkCommandBuffer> onRenderEvents;

			/// COMMAND POOL FOR RENDER COMMANDS
			MvkCommandPool& commandPool;

			void Disposable() {
				vkDeviceWaitIdle(mvkInstance.logicalDevice);

				for (size_t i = 0; i < swapChain_inFlightFences.size(); i++) {
					vkDestroySemaphore(mvkInstance.logicalDevice, swapChain_imageAvailableSemaphores[i], nullptr);
					vkDestroySemaphore(mvkInstance.logicalDevice, swapChain_renderFinishedSemaphores[i], nullptr);
					vkDestroyFence(mvkInstance.logicalDevice, swapChain_inFlightFences[i], nullptr);
				}
			...
			void CreateSwapChainSyncObjects() {
				...
				for (size_t i = 0; i < swapChain.swapChainImages.size(); i++) {
					if (vkCreateSemaphore(mvkInstance.logicalDevice, &semaphoreInfo, nullptr, &swapChain_imageAvailableSemaphores[i]) != VK_SUCCESS ||
						vkCreateSemaphore(mvkInstance.logicalDevice, &semaphoreInfo, nullptr, &swapChain_renderFinishedSemaphores[i]) != VK_SUCCESS ||
						vkCreateFence(mvkInstance.logicalDevice, &fenceInfo, nullptr, &swapChain_inFlightFences[i]) != VK_SUCCESS)
			...
			void BeginRecordCommandBuffer(VkCommandBuffer commandBuffer, const VkClearValue clearColor, const VkClearValue depthStencil, VkImageView& renderTarget, VkImage& renderImageTarget, VkExtent2D renderArea) {
				...
				vkCmdBeginRenderingEKHR(mvkInstance.instance, commandBuffer, &dynamicRenderInfo);
			...
			void EndRecordCommandBuffer(VkCommandBuffer commandBuffer, const VkClearValue clearColor, const VkClearValue depthStencil, VkImageView& renderTarget, VkImage& renderImageTarget, VkExtent2D renderArea) {
				vkCmdEndRenderingEKHR(mvkInstance.instance, commandBuffer);
			...
			void RenderFrame() {
				...
				vkWaitForFences(mvkInstance.logicalDevice, 1, &swapChain_inFlightFences[swapChain.currentFrame], VK_TRUE, UINT64_MAX);
				...
				vkResetFences(mvkInstance.logicalDevice, 1, &swapChain_inFlightFences[swapChain.currentFrame]);
