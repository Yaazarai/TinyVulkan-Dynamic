#ifndef MINIVK_MINIVKBUFFER
#define MINIVK_MINIVKBUFFER
	...
	namespace MINIVULKAN_NS {
		...
		*/
		...
		class MvkBuffer : public MvkObject {
		private:
			void CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VmaAllocationCreateFlags flags) {
				...
				if (vmaCreateBuffer(vmAlloc.GetAllocator(), &bufCreateInfo, &allocCreateInfo, &buffer, &memory, &description) != VK_SUCCESS)
					throw std::runtime_error("MiniVulkan: Could not allocate memory for MvkBuffer!");
			}
		...
		public:
			...
			MvkBuffer(MvkInstance& mvkInstance, MvkMemAlloc& vmAlloc, VkDeviceSize dataSize, VkBufferUsageFlags usage, VmaAllocationCreateFlags flags)
			: mvkInstance(mvkInstance), vmAlloc(vmAlloc), size(dataSize) {
				onDispose += std::callback<>(this, &MvkBuffer::Disposable);
				CreateBuffer(size, usage, flags);
			}

			MvkBuffer(MvkInstance& mvkInstance, MvkMemAlloc& vmAlloc, VkDeviceSize dataSize, MvkBufferType type)
			: mvkInstance(mvkInstance), vmAlloc(vmAlloc), size(dataSize) {
				onDispose += std::callback<>(this, &MvkBuffer::Disposable);

				switch (type) {
					case MvkBufferType::VKVMA_BUFFER_TYPE_VERTEX:
					...
					break;
					case MvkBufferType::VKVMA_BUFFER_TYPE_INDEX:
					...
					break;
					case MvkBufferType::VKVMA_BUFFER_TYPE_UNIFORM:
					...
					break;
					case MvkBufferType::VKVMA_BUFFER_TYPE_INDIRECT:
					...
					break;
					case MvkBufferType::VKVMA_BUFFER_TYPE_STAGING:
					default:
			...
			void TransferBufferCmd(VkQueue graphicsQueue, VkCommandPool commandPool, MvkBuffer& srcBuffer, VkDeviceSize dataSize, VkDeviceSize srceOffset = 0, VkDeviceSize destOffset = 0) {
